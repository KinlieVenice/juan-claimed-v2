...
function Sun({ spin = false }: { spin?: boolean }) {
  return (
    <svg
      viewBox="0 0 100 100"
      className={`h-full w-full ${spin ? "animate-[spin_22s_linear_infinite]" : ""}`}
      aria-hidden
    >
      <g fill="#fcd116">
    ...
    >
      ...
      <header className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 md:px-5 md:py-6">
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <div className="leading-tight">
            <p className="font-display text-lg font-black tracking-tight text-[color:var(--color-ph-blue)] md:text-xl">
              JuanClaimed
            </p>
            <p className="text-[8px] uppercase tracking-[0.18em] text-slate-500 md:text-[10px]">
              Para sa bawat Juan
            </p>
          </div>
        </div>
        <nav className="hidden gap-6 text-sm font-medium text-slate-600 md:flex md:gap-8">
          <a href="#why" className="hover:text-[color:var(--color-ph-blue)]">
            Why
          </a>
          <a href="#how" className="hover:text-[color:var(--color-ph-blue)]">
            How it works
          </a>
          <a href="#team" className="hover:text-[color:var(--color-ph-blue)]">
            Team
          </a>
        </nav>
        <a
          href="/login"
          className="clay-blue hidden px-4 py-2 text-sm font-semibold text-[color:var(--color-ph-blue)] transition hover:translate-y-[-1px] md:inline-block md:px-5 md:py-2.5"
        >
          Open app
        </a>
      </header>
      ...
      <section id="why" className="mx-auto max-w-6xl px-4 py-16 md:px-5 md:py-24">
        <div className="grid gap-8 md:grid-cols-2 md:gap-14">
          ...
          <div className="space-y-4 text-sm leading-relaxed text-slate-600 md:space-y-5 md:text-base lg:text-lg">
            <p>
              To claim a benefit, you first need to know it exists. But there's no single source of
              truth — we rely on word-of-mouth, a neighbor's story, a viral Facebook post.
            </p>
            <p>
              Then the requirements shift branch to branch. One office accepts photocopies, another
              demands originals. You show up, fall short, and get turned away — for a benefit you
              actually qualify for.
            </p>
            <p>
              Even when approved, utilization fails. A senior forgets the booklet at the grocery —
              no discount. JuanClaimed exists so{" "}
              <em className="font-semibold not-italic text-[color:var(--color-ph-blue)]">
                no Juan gets turned away at the counter
              </em>
              .
            </p>
          </div>
          ...
          {[
            {
              ...
              ic: "🔍",
              t: "No single source",
              d: "Benefits live in scattered memos, LGU pages, and hearsay.",
            },
            {
              c: "clay-red",
              ic: "📄",
              t: "Shifting requirements",
              d: "Photocopy vs original, this branch vs that — no truth.",
            },
            {
              c: "clay-yellow",
              ic: "🎫",
              t: "Failed utilization",
              d: "Approved, but the discount slips because of one missing card.",
            },
          ].map((x) => (
            <div key={x.t} className={`${x.c} p-5 transition hover:-translate-y-1 md:p-7`}>
              <div className="clay mb-3 grid h-10 w-10 place-items-center text-xl md:mb-4 md:h-12 md:w-12 md:text-2xl">
                {x.ic}
              </div>
              <p className="font-display text-lg font-bold text-slate-900 md:text-xl">{x.t}</p>
              <p className="mt-1 text-xs leading-relaxed text-slate-700/90 md:mt-2 md:text-sm">
                {x.d}
              </p>
            </div>
          ))}
      ...
      <section id="how" className="mx-auto max-w-6xl px-4 py-16 md:px-5 md:py-20">
        <div className="max-w-2xl">
          ...
          <h2 className="mt-4 font-display text-3xl font-black leading-[1.05] tracking-tight text-slate-900 md:mt-5 md:text-4xl lg:text-6xl">
            One login. A quiz that gets{" "}
            <span className="text-[color:var(--color-ph-red)]">smarter</span> as you answer.
          </h2>
        </div>
          ...
          {[
            {
              n: "01",
              t: "Login with eGov",
              d: "Sign in with your existing eGov account. Basic info like name, birthdate, and address auto-fills from your eGov profile.",
              c: "clay-blue",
            },
            {
              n: "02",
              t: "Answer the general fields",
              d: "A short quiz covers the global tagging fields — age, employment, household, plus location from region down to barangay.",
              c: "clay",
            },
            {
              n: "03",
              t: "Progressive follow-ups",
              d: "Based on your answers, the system asks only the extra LGU or agency-specific questions that unlock more benefits.",
              c: "clay-yellow",
            },
            {
              n: "04",
              t: "Your full eligibility list",
              d: "See every benefit you qualify for — national to barangay — with exact requirements and how to utilize each one.",
              c: "clay-red",
            },
          ].map((s) => (
            <li key={s.n} className={`${s.c} p-5 md:p-7`}>
              <div className="flex items-start gap-3 md:gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs leading-relaxed text-slate-700 md:mt-2 md:text-sm">
                    {s.d}
                  </p>
                </div>
              </div>
        ...
        <div className="clay mt-12 p-6 md:mt-16 md:p-8 lg:p-12">
          ...
          <div className="mt-8 grid gap-6 md:mt-10 md:gap-8 md:grid-cols-2">
            <div>
              <p className="font-display text-base font-bold text-[color:var(--color-ph-blue)] md:text-lg">
                Scoped agents
              </p>
              <p className="mt-1 text-xs leading-relaxed text-slate-600 md:mt-2 md:text-sm">
                Superadmins invite agents with a scope. <strong>National</strong> scope is for
                government departments, agencies, and offices (any nationwide body). Below that:{" "}
                <strong>Region</strong>, <strong>Province</strong>,<strong> District</strong>,{" "}
                <strong>City/Municipality</strong>, and <strong>Barangay</strong>. Each agent only
                manages benefits inside their scope.
              </p>
            </div>
            <div>
              <p className="font-display text-base font-bold text-[color:var(--color-ph-red)] md:text-lg">
                Tagging fields
              </p>
              <p className="mt-1 text-xs leading-relaxed text-slate-600 md:mt-2 md:text-sm">
                Agents create tags like{" "}
                <code className="rounded bg-slate-100 px-1.5 py-0.5 text-xs">age</code> paired with
                a plain-language question ("What's your age?"). Global tags are shared; LGU-specific
                tags stay local.
              </p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="mt-1 text-xs leading-relaxed text-slate-600 md:mt-2 md:text-sm">
                Global fields run first. Then the engine walks down from national to barangay, only
                asking specialized questions that could unlock additional benefits for that user.
              </p>
            </div>
          </div>
        </div>
      ...
      <section id="team" className="mx-auto max-w-6xl px-4 py-16 md:px-5 md:py-20">
        ...
        <div className="mt-10 grid gap-4 md:mt-12 md:gap-5 md:grid-cols-2">
          <div className="clay-blue p-6 md:p-8">
            <p className="text-[10px] font-bold uppercase tracking-widest text-[color:var(--color-ph-blue)] md:text-xs">
              Developers
            </p>
          ...
          <div className="relative flex flex-col items-start justify-between gap-4 md:flex-row md:items-center md:gap-6">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-700 md:mt-2 md:text-sm lg:text-base">
                Login with eGov and get your personalized benefits list in minutes.
              </p>
            </div>
            <a
              href="/login"
      ...
      <footer className="border-t border-slate-200/70 bg-white/60 backdrop-blur">
        <div className="mx-auto flex max-w-6xl flex-col items-start justify-between gap-4 px-4 py-8 md:flex-row md:items-center md:gap-6 md:px-5 md:py-10">
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <div>
              <p className="font-display text-base font-black text-[color:var(--color-ph-blue)] md:text-lg">
                JuanClaimed
              </p>
          ...
          <p className="text-[10px] leading-relaxed text-slate-500 md:text-right md:text-xs">
            ...
            <span className="font-semibold text-slate-700">
              eGovHackathon 2026: Collaborative Solutions for a Better Philippines
            </span>
      ...
      <div className="relative z-10 grid min-w-0 items-center gap-8 md:gap-12 md:grid-cols-[1.15fr_1fr]">
        <div className="min-w-0">
          ...
          <p className="mt-5 max-w-xl break-words text-sm leading-relaxed text-slate-600 md:mt-7 md:text-base lg:text-xl">
            Log in with your eGov account, answer a short progressive quiz, and instantly see every
            government benefit you're eligible for — from national programs down to your barangay.
          </p>
        ...
        {/* Right: floating benefit card mock */}
        <BenefitPreview />
      </div>
    </section>
  );
  ...
  return (
    <div className="relative border-y border-slate-200/60 bg-white/40 py-3 backdrop-blur md:py-5">
      <div className="flex gap-3 overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)] md:gap-4">
        <div className="flex shrink-0 animate-[marquee_38s_linear_infinite] gap-3 md:gap-4">
          {items.map((b, i) => (
            <span
              key={i}
        ...
        <div
          aria-hidden
          className="flex shrink-0 animate-[marquee_38s_linear_infinite] gap-3 md:gap-4"
        >
          {items.map((b, i) => (
            <span
              key={`b${i}`}
...
function BenefitPreview() {
  ...
  const items = [
    {
      ...
      scope: "National",
      note: "You're eligible — ₱20,000/yr",
    },
    { emoji: "💊", title: "PhilHealth Konsulta", scope: "National", note: "Free primary care" },
    { emoji: "🌾", title: "Farmer's Fuel Subsidy", scope: "Province", note: "₱3,000 quarterly" },
  ...
  return (
    <div className="relative mx-auto w-full max-w-xs md:max-w-md">
      <div className="clay p-3 sm:p-4 md:p-6 lg:p-7">
        <div className="flex items-center justify-between gap-2 md:gap-3">
          <div className="min-w-0">
            <p className="text-[8px] font-bold uppercase tracking-widest text-slate-400 md:text-[10px]">
              Your matches
            </p>
            <p className="truncate font-display text-base font-black text-slate-900 sm:text-lg md:text-2xl">
              12 benefits found
            </p>
          </div>
          <div className="clay-yellow grid h-9 w-9 flex-shrink-0 place-items-center text-base md:h-11 md:w-11 md:text-xl">
            ✨
          </div>
        </div>
          ...
          {items.map((it, i) => (
            <div
              ...
              className={`flex items-center gap-3 rounded-2xl p-2 transition-all duration-500 md:gap-4 md:p-3 ${
                i === active
                  ? "clay-blue scale-[1.02]"
                  : "bg-white/70 shadow-[inset_0_0_0_1px_rgba(0,56,168,0.06)]"
              }`}
            >
              <div className="clay grid h-9 w-9 flex-shrink-0 place-items-center text-base md:h-11 md:w-11 md:text-xl">
                {it.emoji}
              </div>
              <div className="min-w-0 flex-1">
                <p className="truncate font-display text-sm font-bold text-slate-900 md:text-base">
                  {it.title}
                </p>
        ...
        <div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-3 text-[10px] md:mt-5 md:pt-4 md:text-xs">
          ...
          <span className="font-bold text-[color:var(--color-ph-blue)]">View all →</span>
        </div>
      </div>
